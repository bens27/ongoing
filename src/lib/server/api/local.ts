...
import { entryHistoryResponse } from './history';
import { failure, resolveEntryRef } from './support';
import { attachmentError, AttachmentService, logoListingResponse } from '$lib/server/attachments';
...
const routes: { method: string; pattern: RegExp; handle: Handler }[] = [
  {
    method: 'GET',
    pattern: /^\/api\/entries\/([^/]+)\/([^/]+)\/history$/,
    handle: ({ repository, params, parameter }) =>
      entryHistoryResponse(repository, params[0], params[1], parameter)
  },
  {
    method: 'GET',
  ...
  },
  {
    method: 'GET',
    pattern: /^\/api\/logos$/,
    handle: async ({ attachments, parameter }) =>
      json(await logoListingResponse(attachments, parameter('field') ?? undefined))
  },
  {
    method: 'GET',
    pattern: /^\/api\/health$/,
...
export function createLocalApi(env: Record<string, string | undefined> = process.env): LocalApi {
  ...
  return {
    ...
    async request(path, init = {}) {
      ...
      const url = new URL(path, 'http://ongoing.local');
