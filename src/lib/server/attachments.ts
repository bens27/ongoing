...
import type { LogoListing } from '$lib/domain/logo-set';
...
export class AttachmentService {
  ...
  async set(entryId: string, field: string, input: unknown, expected: string | null) {
    ...
    await adapter.validateDocument(bundle.document);
  ...
  /** Every project whose identity field holds a valid reference, for consumer-site exports. */
  async listLogos(field = LOGO_FIELD): Promise<LogoListing[]> {
    const definition = this.repository.registry().get(field);
    const adapter = definition?.presentation
      ? richFieldAdapter(definition.presentation.adapter)
      : undefined;
    if (!adapter) return [];
    const listings: LogoListing[] = [];
    for (const entry of this.repository.listEntries({ kind: 'project', includeHidden: true })) {
      const value = entry.attributes[field] as unknown as RichFieldValue | undefined;
      if (!value || value.kind !== adapter.referenceKind) continue;
      let layouts: Record<string, unknown> | undefined;
      try {
        const document = JSON.parse(
          (await this.artifacts.read(value.manifest.hash)).toString('utf8')
        );
        if (document && typeof document === 'object' && document.layouts)
          layouts = document.layouts;
      } catch {
        // A missing artifact leaves the listing without hints; the consumer still gets the reference.
      }
      listings.push({
        id: entry.id,
        slug: entry.slug,
    ...
    return listings.sort((left, right) => left.slug.localeCompare(right.slug));
  }
...
/** The `GET /api/logos` body, shared by the HTTP route and the CLI's in-process transport. */
export async function logoListingResponse(service: AttachmentService, field = LOGO_FIELD) {
  const logos = await service.listLogos(field);
  return {
    schemaVersion: 1,
    field,
    logos: logos.map((logo) => ({
      ...logo,
      manifestUrl: `/api/artifacts/${logo.manifest.hash}`,
      posterUrl: `/api/artifacts/${logo.poster.hash}`
    }))
  };
}
