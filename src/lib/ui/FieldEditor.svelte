<script lang="ts">
  ...
  let {
    ...
    ariaLabel?: string;
    /**
     * A cell commits when focus leaves it. A draft that has to survive a click elsewhere — the
     * overview's next action, edited while switching signal tabs — turns this off and commits
     * only on Enter or an explicit save.
     */
  ...
  // Cancelling unmounts the editor, and the blur that follows must not commit what was typed.
  let cancelled = false;
  ...
  /** Commit the current draft; for an owner with its own save button beside the editor. */
...
<div class="editor" data-type={definition.type}>
  ...
  {:else if longText}
    <textarea
      ...
      use:focusOnMount
      {onkeydown}
      onblur={() => commitOnBlur && !cancelled && commit(draft)}></textarea>
  ...
  {:else}
    <input
      ...
      use:focusOnMount
      {onkeydown}
      onblur={() => commitOnBlur && !cancelled && commit(draft)}
    />
  {/if}
