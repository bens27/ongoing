...
/** A count with thousands separators, as an overview headline reads it: `1,400`, never `1.4k`. */
export function wholeNumber(value: number | null | undefined): string {
  if (value === null || value === undefined || !Number.isFinite(value)) return EM_DASH;
  return value.toLocaleString('en-US', { maximumFractionDigits: 0 });
}

/** One decimal, for a rate such as commits per day. */
export function oneDecimal(value: number | null | undefined): string {
  if (value === null || value === undefined || !Number.isFinite(value)) return EM_DASH;
  return value.toFixed(1);
}

/** `Sep 13` — a date as a chart axis or a footer says it, in UTC so a build date never shifts. */
