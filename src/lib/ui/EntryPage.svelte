<script lang="ts">
  ...
  import ProjectFactsheet from './project/ProjectFactsheet.svelte';
  import { safeReturnTo } from './safe-path';
  import { attentionTone, ringTone, VIEW_LABELS } from './views';

  /**
   * The fact sheet. A project gets the wide, section-composed layout in `ProjectFactsheet`; every
   * other kind keeps this generic body: identity at the top, then everything `EntryFacts` renders
   * — decisions, why the entry appears in an attention view with the thresholds behind it,
   * relations, the trend, and one panel per provider that left data. Every field is editable in
   * place.
   */
  ...
  // The inventory the fact sheet was opened from — its query, sort, columns, and selected entry —
  // so leaving returns to the same list rather than a fresh one. A direct link carries no `from`
  // and falls back to a plain reset; `safeReturnTo` refuses anything that is not an in-app path,
  // including a protocol-relative `//host` that would leave the app.
  let backHref = $derived(safeReturnTo(page.url.searchParams.get('from')));
</script>
...
{#if !entry}
  <p class="missing" role="alert">
    ...
    catalog. <a href={backHref}>Back to the inventory</a>.
  </p>
{:else if entry.kind === 'project'}
  <ProjectFactsheet {entry} {catalog} {backHref} />
{:else}
  <header class="head">
    <div class="identity">
      {#if editingName}
