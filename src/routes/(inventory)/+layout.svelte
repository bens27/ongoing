<script lang="ts">
  ...
  import { setCatalogContext, setShellContext } from '$lib/ui/context';
  ...
  setShellContext({ openPalette: () => (paletteOpen = true) });
...
<style>
  ...
  .status {
    /* Bottom centre: the detail panel's footer lives bottom right, and a toast must not sit on
       the control that opens the field inspector. */
    position: fixed;
    left: 50%;
    bottom: var(--space-4);
