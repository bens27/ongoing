...
| default        | `http://127.0.0.1:4173`, then the in-process path    |
...
Loopback needs no login; a non-loopback listener requires `ONGOING_ACCESS_SECRET` unless `ONGOING_DISABLE_AUTH=true`, which is a trusted-LAN escape hatch, not a default ([docs/auth.md](auth.md)). `ongoing login <secret>`
...
ongoing history <entry> --metric <name> [--days 90] [--json]
                                      dated observations, availability, and freshness
...
## Dated metric history

`ongoing history` reads one entry and metric through
`GET /api/entries/:kind/:slug/history?metric=github_stars&days=90`. The HTTP and local transports
return the same `EntryHistory` contract; `--json` prints it verbatim. The human output prints the
window, provider status, freshness, and each dated value. Entry references use the same resolution
as `ongoing get`; use `project/<slug>` when a slug occurs in multiple kinds.

```sh
ongoing history project/sidecar --metric github_stars --days 90 --json
ongoing history . --metric loc_code --days 30 --local
curl 'http://localhost:4173/api/entries/project/sidecar/history?metric=github_stars&days=90'
```

`metric` is required and comes from `snapshotMetrics` in `src/lib/domain/metrics.ts`:
`github_stars`, `loc_code`, `github_traffic_views`, `github_traffic_unique_visitors`,
`github_traffic_clones`, `github_traffic_unique_cloners`, `github_open_issues`, `github_open_prs`.
`days` defaults to 90 and accepts integers from 1 through 365. The window includes today and the
preceding `days - 1` UTC calendar dates, with inclusive `from` and `to` dates. These are stored
observations of the named metric at capture time; traffic observations retain the collector's
traffic totals, not daily traffic counts. Changing `days` selects capture dates, not a different
traffic aggregation window. Daily commit history is not collected by this capability.

The response contains:

| Property       | Meaning                                                                     |
| -------------- | --------------------------------------------------------------------------- |
| `entry`        | `{ id, kind, slug, name }`                                                  |
| `metric`       | The validated snapshot metric name                                          |
| `window`       | `{ days, from, to }`, inclusive UTC calendar dates                          |
| `observations` | Ascending `{ capturedOn, value }` points; `value` is a number or `null`     |
| `availability` | `{ state, provider, providerStatus, reason }`, described below              |
| `freshness`    | `{ state, scannedAt, latestSnapshotOn, maxAgeHours }`                       |
| `omittedDates` | Count of stored rows with missing or invalid dates, omitted from the result |

The response preserves zero values, single observations, constant series, sparse dates, and null
values. Missing days are not filled, invalid/non-finite values become `null`, and future dated
observations are excluded. No collection runs during this read, and no new schema is needed.

| Outcome                                                                              | HTTP/local status and result                                                      |
| ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------- |
| Unknown project slug                                                                 | 404 with `{ error }`                                                              |
| Unsupported kind (including technology), missing/unsupported metric, invalid days    | 400 with `{ error }`                                                              |
| Available provider with numeric observations                                         | 200; `availability.state = "available"`                                           |
| Available provider with no numeric observations in the window                        | 200; `availability.state = "empty"` (null points, if any, remain present)         |
| Disabled/unavailable provider, failed collector, or no known collection availability | 200; `availability.state = "unavailable"`, with a reason and any retained history |

`provider` is `github` or `loc`. `providerStatus` preserves `available`, `unavailable`,
`unauthenticated`, or `rate_limited`; `unknown` means no recorded availability. A provider excluded
from the host's active set is `unavailable`, with an explicit disabled-or-unavailable reason.
An unresolved error from the relevant collector also marks availability unavailable; unrelated
collector errors do not. Availability does not erase previously collected observations.

Freshness is independent of availability and of whether this window contains observations.
`scannedAt` is the relevant successful collector timestamp (hosting, traffic, or LOC), and
`freshness.state` is `fresh` up to the existing 72-hour threshold, `stale` after it, or `unknown`
for missing, invalid, or future timestamps. `latestSnapshotOn` is the newest valid snapshot date
for this metric up to today, including observations outside the requested window, or `null`.
It describes snapshot recency; `scannedAt` describes source freshness. Consumers can cache by
entry, metric, window, and these freshness values and refresh after collection.

This focused read leaves `ongoing show` compatible and does not add history to the initial
inventory payload or fetch the legacy `/api/projects` catalog. CLI reference resolution reads the
entries catalog, then requests history only for the selected entry.
...
### Logo sets for consumer sites

A website that shows project logos — Haplab, OpenTangle, a product site — pulls the current set in one command and embeds it with one script. The layout is fixed so every consumer shares the same helper:

```sh
ongoing logos                                   # every project with a logo, and its revision
ongoing logos export --output public/identity   # all logos
ongoing logos export --output public/identity sidecar td tasks   # only these slugs (or IDs)
```

```
public/identity/
  logos.json            manifest: schemaVersion 1, kind impressions.logo.set, runtime, logos by slug
  impressions-logo.js   self-contained viewer runtime (global ImpressionsLogo)
  logo-embed.js         dependency-free ES module: LogoSet.load(...).mount(host, slug, options)
  <slug>/logo.json      the portable impressions.logo document (full 3D)
  <slug>/logo.png       the transparent 1024px poster (static)
```

Each manifest entry carries the acknowledged `revision` and, when the document has one, its `layouts` (card product-name placement), so a page can lay out a card before fetching the document. Output is deterministic: an unchanged catalog rewrites nothing, and the JSON result lists `changed` files, so a site that commits the set sees a diff only when a logo moved. A requested project without a logo is reported under `missing` rather than failing the pull, and a `<slug>/` directory left by an earlier pull is reported under `stale` rather than deleted, since the output may be a site's shared asset directory. `GET /api/logos` is the same listing over HTTP with `/api/artifacts/<hash>` URLs for dynamic consumers on the private network; public sites copy the set into their own deployment rather than sending visitors to the tailnet.

Embedding, from a page in the same directory tree:

```html
<div data-impressions-logo="sidecar" data-mode="live"></div>
<script type="module">
  import { LogoSet } from './identity/logo-embed.js';
  const logos = await LogoSet.load('./identity/logos.json');
  logos.mountAll(); // or logos.mount(element, 'sidecar', { mode: 'live', rotating: true })
</script>
```

`mode: 'static'` (the default) shows the poster only. `mode: 'live'` shows the poster at once and replaces it with the real renderer after the runtime loads from the same directory; a WebGL failure keeps the poster. The runtime comes from the private `@impressions/logo` overlay (see `src/lib/logo-overlay.d.ts`), which public clones do not install — `logos export` needs it, everything else works without it. Upgrading the viewer means re-vendoring that tarball and re-running the export.
...
[server]
port = 4173
database = "~/.local/share/ongoing/ongoing.sqlite"
...
```sh
ongoing serve --data-dir ~/.local/share/ongoing   # foreground, fresh catalog on port 4173
...
| `history`                   | `GET /api/entries/:kind/:slug/history?metric=&days=`    |
