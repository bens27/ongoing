...
import { parseHistoryRequest, type EntryHistory } from '../src/lib/domain/entry-history';
import { snapshotMetrics } from '../src/lib/domain/metrics';
import type { HostAdapter, HostService } from '../src/lib/host/adapter';
import {
  buildLogoSetManifest,
  LOGO_SET_FILES,
  selectLogos,
  type LogoListing
} from '../src/lib/domain/logo-set';
...
// Read from package.json so the CLI can never drift from the published version again.
...
async function commandHistory(client: ApiClient, args: Args): Promise<void> {
  const token = args.positional[0];
  if (!token || args.positional.length !== 1)
    throw new CliError('Usage: ongoing history <entry> --metric <metric> [--days 90] [--json]');
  const request = parseHistoryRequest(args.flags.get('metric'), args.flags.get('days'));
  const entry = await resolveEntry(client, token);
  const history = await client.request<EntryHistory>(
    `/api/entries/${encodeURIComponent(entry.kind)}/${encodeURIComponent(entry.slug)}/history`,
    { query: { metric: request.metric, days: String(request.days) } }
  );
  if (flag(args, 'json')) return printJson(history);
  out(`${bold(history.entry.name)} · ${history.metric}`);
  out(`${history.window.from} – ${history.window.to} (UTC, ${history.window.days} days)`);
  out(
    `History: ${history.availability.state} · ${history.availability.provider}: ${history.availability.providerStatus}`
  );
  if (history.availability.reason) out(history.availability.reason);
  out(
    `Freshness: ${history.freshness.state} · collected ${history.freshness.scannedAt ?? 'never'} · latest snapshot ${history.freshness.latestSnapshotOn ?? 'none'}`
  );
  if (history.omittedDates)
    out(`${history.omittedDates} observation(s) omitted: missing or invalid date`);
  for (const point of history.observations) out(`${point.capturedOn}  ${point.value ?? 'unknown'}`);
}
...
interface LogoListingResponse {
  schemaVersion: number;
  field: string;
  logos: (LogoListing & { manifestUrl: string; posterUrl: string })[];
}

/** The viewer runtime and embed helper ship with the private @impressions/logo overlay. */
function logoRuntimeFiles(): { script: string; embed: string; version: string } {
  const root = join(REPO, 'node_modules', '@impressions', 'logo');
  const script = join(root, 'dist', 'standalone.js');
  const embed = join(root, 'dist', 'embed.js');
  if (!existsSync(script) || !existsSync(embed))
    throw new CliError(
      `Logo export needs the private @impressions/logo overlay in ${REPO} (see src/lib/logo-overlay.d.ts); list and attachment commands work without it.`
    );
  const version = String(JSON.parse(readFileSync(join(root, 'package.json'), 'utf8')).version);
  return { script, embed, version };
}

/**
 * `ongoing logos` lists every project's current logo; `ongoing logos export --output DIR` writes
 * the set a consumer site embeds: logos.json, <slug>/logo.json, <slug>/logo.png, the viewer runtime
 * and the embed helper. Positional slugs or IDs narrow the set; a requested project without a logo is
 * reported, not fatal, so a site's pull keeps working while an identity is still being chosen.
 */
async function commandLogos(client: ApiClient, args: Args): Promise<void> {
  const action = args.positional.shift() ?? 'list';
  if (!['list', 'export'].includes(action))
    throw new CliError(
      'Usage: ongoing logos [list] [slug|id]... | logos export --output <directory> [slug|id]...'
    );
  const field = option(args, 'field') ?? 'identity.logo';
  const listing = await client.request<LogoListingResponse>('/api/logos', { query: { field } });
  const requested = [...args.positional, ...(option(args, 'project') ?? '').split(',')]
    .map((value) => value.trim())
    .filter(Boolean);
  const { selected, missing } = selectLogos(listing.logos, requested);
  if (action === 'list') {
    if (flag(args, 'json')) return printJson({ ...listing, logos: selected, missing });
    if (!selected.length) return out('No project has a logo yet.');
    for (const logo of selected)
      out(`${logo.slug.padEnd(18)} ${logo.revision.slice(0, 12)}  ${logo.name}`);
    for (const key of missing) out(`${yellow('!')} ${key} has no logo`);
    return;
  }
  const output = option(args, 'output');
  if (!output) throw new CliError('logos export requires --output <directory>');
  const runtime = logoRuntimeFiles();
  const directory = resolvePath(output);
  mkdirSync(directory, { recursive: true });
  const written: string[] = [];
  const changed: string[] = [];
  const write = (file: string, bytes: string | Buffer) => {
  ...
  for (const logo of selected) {
    const current = await client.request<AttachmentRead>(
      `/api/attachments/${encodeURIComponent(logo.id)}/${encodeURIComponent(field)}`
    );
    if (!current.poster || current.document === undefined || current.revision !== logo.revision)
      throw new CliError(`${logo.slug} changed while exporting; run the export again.`);
    write(LOGO_SET_FILES.document(logo.slug), `${JSON.stringify(current.document, null, 2)}\n`);
    write(LOGO_SET_FILES.poster(logo.slug), Buffer.from(current.poster.base64, 'base64'));
  }
  // Directories from an earlier pull that no longer belong to the set are reported, not deleted:
  // the output may be a site's shared asset directory.
  const stale = existsSync(directory)
    ? readdirSync(directory, { withFileTypes: true })
        .filter(
          (entry) =>
            entry.isDirectory() &&
            !selected.some((logo) => logo.slug === entry.name) &&
            existsSync(join(directory, entry.name, 'logo.json'))
        )
        .map((entry) => entry.name)
        .sort()
    : [];
  const manifest = buildLogoSetManifest(selected, runtime.version);
  write(LOGO_SET_FILES.manifest, `${JSON.stringify(manifest, null, 2)}\n`);
  write(LOGO_SET_FILES.runtime, readFileSync(runtime.script));
  write(LOGO_SET_FILES.embed, readFileSync(runtime.embed));
  const result = {
    directory,
    field,
    logos: Object.fromEntries(selected.map((logo) => [logo.slug, logo.revision])),
    missing,
    stale,
    files: written,
    changed
  ...
  out(
    `${green('✓')} exported ${selected.length} logo${selected.length === 1 ? '' : 's'} to ${directory}`
  );
  for (const logo of selected) out(`  ${logo.slug.padEnd(18)} ${logo.revision.slice(0, 12)}`);
  out(
    changed.length
  ...
  for (const slug of stale)
    out(
      `${yellow('!')} ${slug}/ is from an earlier pull and is no longer in the set; remove it by hand`
    );
}
...
${bold('Reading')}
  ...
  history <entry> --metric <metric>     dated observations with availability and freshness
      --days <1-365>                    UTC calendar window including today (default 90)
      --json                            identical history response to the API
      metrics: ${snapshotMetrics.join(', ')}
...
${bold('Catalog')}
  ...
  logos [slug|id]...                    every project's current logo and revision
  logos export --output <dir> [slug|id]...
                                        write a consumer logo set: logos.json, <slug>/logo.{json,png},
                                        the viewer runtime and the embed helper
...
async function main(argv: string[]): Promise<void> {
  ...
  switch (command) {
    ...
    case 'history':
      return commandHistory(client, args);
    case 'stacks':
    ...
    case 'logos':
      return commandLogos(client, args);
    case 'attachment':
    case 'attachments':
