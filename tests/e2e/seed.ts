...
await repository.updateMetrics(alpha.id, {
  ...
  locScannedAt: ago(1 * MINUTE),
  tdScannedAt: ago(1 * MINUTE),
  githubScannedAt: ago(1 * MINUTE),
  githubTrafficScannedAt: ago(1 * MINUTE)
});
// A configured public site: the owned website destination the panel links to, distinct from any
// `url`-typed field someone registers later.
await repository.updateWebsite(alpha.id, {
  included: true,
  slug: 'alpha',
  name: 'Alpha',
  tagline: 'A parser with opinions',
  description: 'Alpha parses catalog queries.',
  category: 'Tool',
  year: '2026',
  stack: 'TypeScript',
...
await repository.saveSnapshot({
  projectId: alpha.id,
  metric: 'github_stars',
  capturedOn: agoDate(80),
  value: 100
});
await repository.saveSnapshot({
  projectId: alpha.id,
  metric: 'github_stars',
  capturedOn: agoDate(60),
  value: 105
});
await repository.saveSnapshot({
  projectId: alpha.id,
...
await repository.saveSnapshot({
  projectId: alpha.id,
  metric: 'github_stars',
  capturedOn: agoDate(7),
  value: 118
});
await repository.saveSnapshot({
  projectId: alpha.id,
  metric: 'github_stars',
  capturedOn: agoDate(0),
  value: 120
});
await repository.saveSnapshot({
  projectId: alpha.id,
  metric: 'loc_code',
  capturedOn: agoDate(70),
  value: 4_500
});
await repository.saveSnapshot({
  projectId: alpha.id,
  metric: 'loc_code',
  capturedOn: agoDate(35),
  value: 4_700
});
await repository.saveSnapshot({
  projectId: alpha.id,
  metric: 'loc_code',
  capturedOn: agoDate(2),
  value: 4_812
});
await repository.saveSnapshot({
  projectId: alpha.id,
